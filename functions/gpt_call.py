...
def text_call(content):
    ...
    completion = openai_client.responses.create(
        ...
        input=[
            {
                "role": "system",
                "content": "You are an AI that generates extremely detailed notes while maintaining continuity across sections."
            },
            {
                "role": "user",
                "content": f"""
    Write clear, detailed notes in valid HTML to be inserted into a Jinja template. Output valid HTML without triple backticks, code fences, or extra symbols like >>>. Use headings (<h2>, <h3>), paragraphs (<p>), bold (<strong>) or italics (<em>) for emphasis, and ordered (<ol>) or unordered (<ul>) lists where appropriate. At the end of each complete element of HTML, insert '<!-- END_SECTION -->' Present facts directly without introductory framing, explanations of importance, or general overviews. Avoid addressing an audience or using terms like 'we,' 'you,' or 'must.' Use natural, straightforward language without formal phrasing. Stick closely to the provided content, clarifying only when necessary, and do not expand beyond the mentioned information.
...
def question_call(question, selection):
    ...
    model="gpt-4o",
    input=[
        {"role": "system", "content": "You are an AI that generates extremely brief conceptual answers to questions while maintaining continuity across sections when relevant. Prior responses should be considered only if they directly relate to the new question. If the new question is unrelated, answer independently without incorporating previous answers."},
        {"role": "user", "content": f"""
